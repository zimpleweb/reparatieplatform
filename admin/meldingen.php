    ...
    <div class="admin-card" style="padding:0;overflow:hidden;">
      ...
      <?php else: ?>
        <table class="m-tabel">
          <thead>
            <tr>
              <th class="m-check-col"><input type="checkbox" id="selectAll" title="Alles selecteren" /></th>
              <th>Datum</th>
              <th>Type</th>
              <th>Casenummer</th>
              <th>Apparaat</th>
          ...
          ?>
            <tr class="<?= $gelezen ? '' : 'rij-ongelezen' ?>">
              <td class="m-check-col"><input type="checkbox" name="selectie[]" value="<?= (int)$m['id'] ?>" class="melding-check" /></td>
              ...
              <td>
                ...
                <?php if (!$gelezen): ?><span class="ongelezen-dot" title="Ongelezen"></span><?php endif; ?>
              </td>
              <td>
                <?php if ($m['aanvraag_id_link']): ?>
                  <a href="#" data-lees="<?= (int)$m['id'] ?>"
              ...
              <td class="m-apparaat">
                ...
                <?php if (!empty($m['email'])): ?><br><span><?= h($m['email']) ?></span><?php endif; ?>
              </td>
              <td class="m-tekst">
                <div class="m-actie"><?= h($m['actie']) ?></div>
                <?php if (!empty($m['opmerking'])): ?><div class="m-opmerking"><?= nl2br(h($m['opmerking'])) ?></div><?php endif; ?>
              </td>
              <td><span class="m-door <?= door_css($m['gedaan_door']) ?>"><?= h($m['gedaan_door']) ?></span></td>
              <td>
                <?php if (!$gelezen): ?>
        ...
        <div style="padding:.75rem 1.25rem;font-size:.775rem;color:#94a3b8;border-top:1px solid #f1f5f9;">
          ...
          <?php if (!$filterArchief): ?>&bull; <strong><?= $ongelezen_totaal ?></strong> ongelezen<?php endif; ?>
        </div>
      <?php endif; ?>
    </div>
  </form>
...
function updateBulkBalk() {
  ...
  bulkBalk.style.display = n > 0 ? 'flex' : 'none';
}
if (selectAll) {
