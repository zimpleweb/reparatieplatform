...
<head>
  ...
  <style>
    ...
    .pw-eye { position: absolute; right: .65rem; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: var(--muted); font-size: 1rem; padding: 0; line-height: 1; }
    ...
    .badge-you { background: #d6f0eb; color: #287864; font-size: .7rem; font-weight: 700; padding: .15rem .5rem; border-radius: 99px; margin-left: .5rem; vertical-align: middle; }
    .badge-2fa-on  { background: #dcfce7; color: #15803d; font-size: .7rem; font-weight: 700; padding: .15rem .5rem; border-radius: 99px; margin-left: .4rem; vertical-align: middle; }
    .badge-2fa-off { background: #fef9c3; color: #854d0e; font-size: .7rem; font-weight: 700; padding: .15rem .5rem; border-radius: 99px; margin-left: .4rem; vertical-align: middle; }
    ...
    .modal-box { background: white; border-radius: 16px; padding: 2rem; width: 100%; max-width: 420px; box-shadow: 0 20px 60px rgba(0,0,0,.2); }
    ...
    .btn-admin-sm { padding: .35rem .75rem; border-radius: 8px; font-size: .8rem; font-weight: 600; cursor: pointer; transition: all .15s; border: none; }
    .btn-admin-outline { border: 1.5px solid var(--border); background: white; color: var(--ink); }
    .btn-admin-outline:hover { border-color: var(--accent); color: var(--accent); }
    .btn-admin-danger { background: #fee2e2; color: #b91c1c; border: 1.5px solid #fecaca; }
    .btn-admin-danger:hover { background: #fecaca; }
    .btn-admin-warning { background: #fef9c3; color: #854d0e; border: 1.5px solid #fef08a; }
...
<div class="adm-page">
  ...
  <?php if ($successMsg): ?><div class="alert alert-success" style="margin-bottom:1.5rem;">&#10003; <?= h($successMsg) ?></div><?php endif; ?>
  <?php if ($errorMsg): ?><div class="alert alert-error" style="margin-bottom:1.5rem;">&#9888; <?= h($errorMsg) ?></div><?php endif; ?>

  <div class="admin-card" style="margin-bottom:2rem;">
    <h2>Bestaande admin-accounts</h2>
    <table class="admin-table">
      <thead>
        <tr><th>#</th><th>Gebruikersnaam</th><th>E-mailadres</th><th>2FA</th><th>Aangemaakt op</th><th>Acties</th></tr>
      </thead>
      <tbody>
      ...
      <tr>
        ...
        <td><strong><?= h($a['username']) ?></strong><?php if ($isJij): ?><span class="badge-you">jij</span><?php endif; ?></td>
        <td style="font-size:.85rem;"><?= !empty($a['email']) ? h($a['email']) : '<span style="color:var(--muted);font-style:italic;">—</span>' ?></td>
        <td><?= $has2fa ? '<span class="badge-2fa-on">Aan</span>' : '<span class="badge-2fa-off">Uit</span>' ?></td>
        <td style="font-size:.8rem;color:var(--muted);"><?= h($a['created_at'] ?? '—') ?></td>
        <td>
          <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
            <button type="button" onclick="openEmailModal(<?= (int)$a['id'] ?>, '<?= h(addslashes($a['username'])) ?>', '<?= h(addslashes($a['email'] ?? '')) ?>')" class="btn-admin-sm btn-admin-outline">E-mail</button>
            <button type="button" onclick="openPwModal(<?= (int)$a['id'] ?>, '<?= h(addslashes($a['username'])) ?>')" class="btn-admin-sm btn-admin-outline">Wachtwoord</button>
            <?php if ($isJij && !$has2fa): ?>
              <a href="<?= BASE_URL ?>/admin/2fa-setup.php" class="btn-admin-sm btn-admin-warning" style="text-decoration:none;">2FA instellen</a>
            ...
            <?php endif; ?>
            <?php if (!$isJij): ?>
              <form method="POST" style="margin:0;" onsubmit="return confirm('Admin \"<?= h(addslashes($a['username'])) ?>\" verwijderen?');">
                <input type="hidden" name="csrf" value="<?= csrf() ?>">
  ...
  <div class="admin-card">
    ...
    <form method="POST" class="form-admin" style="max-width:460px;">
      <input type="hidden" name="csrf"   value="<?= csrf() ?>">
      <input type="hidden" name="action" value="create">
      <div class="field"><label>Gebruikersnaam *</label><input type="text" name="username" placeholder="bijv. admin2" autocomplete="off" required /></div>
      <div class="field"><label>E-mailadres <small style="font-weight:400;color:var(--muted);">(optioneel)</small></label><input type="email" name="email" placeholder="admin@voorbeeld.nl" autocomplete="off" /></div>
      <div class="field">
      ...
      <button type="submit" class="btn-primary" style="margin-top:.5rem;">Account aanmaken</button>
    </form>
  </div>
</div><!-- /.adm-page -->
...
<div class="modal-overlay" id="emailModal">
  <div class="modal-box">
    ...
    <form method="POST" class="form-admin">
      ...
      <div class="field"><label>E-mailadres</label><input type="email" name="new_email" id="modalEmail" placeholder="admin@voorbeeld.nl" /></div>
...
function checkStrength(input, targetId) {
  const pw = input.value; const el = document.getElementById(targetId);
  if (!pw) { el.textContent = ''; el.className = 'pw-strength'; return; }
  let score = 0;
  if (pw.length >= 10) score++; if (pw.length >= 14) score++;
